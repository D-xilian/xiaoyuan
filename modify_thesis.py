...
old2 = '本文从系统架构设计、数据模型构建、核心功能实现三个维度展开研究，采用四步法系统呈现了研究结果。'
new2 = '本文从系统架构设计、数据模型构建、核心功能实现三个维度展开研究，并围绕功能模块划分、数据关系建模、关键技术实现和效果分析四个层面进行系统阐述。'
replace_in_para(doc.paragraphs[2], old2, new2)

# ============================================
# 修改点3：引言末尾删除"四步法"表述
# ============================================
print('[修改点3] 修正引言末尾表述...')
old3 = '在论文结构上，本文第二章节概述系统的整体架构与技术方案，第三章节采用四步法系统呈现研究结果，分别从谋篇布局、数据呈现、文字解读和关联升华四个维度展开分析，最后对全文进行总结并提出未来展望。'
new3 = '在论文结构上，第2章介绍系统的整体架构与技术方案，第3章从功能模块设计、数据模型构建、关键技术实现和效果分析四个方面展开论述，最后对全文进行总结并提出未来展望。'
replace_in_para(doc.paragraphs[9], old3, new3)
...
for p in doc.paragraphs:
    t = p.text.strip()
    if t.startswith('3.3.1') and 'RESTful' in t:
        remove_para_element(p)
        print('  已移除3.3.1标题')
        break
for p in doc.paragraphs:
    t = p.text.strip()
    if t.startswith('3.3.2') and '核心API' in t:
        remove_para_element(p)
        print('  已移除3.3.2标题')
        break
